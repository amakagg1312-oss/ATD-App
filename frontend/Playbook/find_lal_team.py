# Search for team ID "LAL" or Lakers data in memory
import ctypes
...
# Search for "LAL" as ASCII
print("Searching for 'LAL' in memory...")
    ...
    while idx != -1:
        ...
        if kern.ReadProcessMemory(hproc, ctypes.c_void_p(addr - 20), data, 100, byref(c_size_t(0))):
            ...
            for offset in range(0, 100, 4):
                ...
                if cnt == 70:
                    print(f"  Found 'LAL' at {hex(addr)}, count=70 at offset {offset}")
                    # Read plays
