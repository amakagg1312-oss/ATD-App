"""Persistent team generation server.

Communicates via stdin/stdout using NDJSON (newline-delimited JSON).
Imports heavy modules once at startup, then handles requests quickly.

Protocol:
  Request:  {"action": "generate", "players": [...], "season": "...", "db_dir": "...", "roles_dir": "...", "project_root": "..."}
  Response: {"type": "progress", "completed": N, "total": M, "player": "..."}
            {"type": "player", "ok": true/false, "player": "...", "profile": {...} or "error": "..."}
            {"type": "done", "success": N, "failed": M}

  Request:  {"action": "ping"}
  Response: {"type": "pong"}

  Request:  {"action": "quit"}
  Response: (server exits)
"""
...
print(json.dumps({"type": "status", "status": "initializing"}), flush=True)
...
# ── Attribute and tendency definitions ─────────────────────────────────────
...
_compute_tendencies = None
_compute_attributes_ml = None
_compute_attribute_family_averages = None
_compute_badge_groups = None
_THREE_POINT_RULES = None
_MID_POST_RULES = None
_DRIBBLE_RULES = None
_DEFENSE_RULES = None


def ensure_modules_loaded(project_root):
    """Lazy-load heavy modules on first use."""
    global _modules_loaded, _compute_tendencies, _compute_attributes_ml
    global _compute_attribute_family_averages, _compute_badge_groups
    global _THREE_POINT_RULES, _MID_POST_RULES, _DRIBBLE_RULES, _DEFENSE_RULES
    ...
    _compute_tendencies = compute_tendencies
    _compute_attributes_ml = compute_attributes_ml
    _compute_attribute_family_averages = compute_attribute_family_averages
    _compute_badge_groups = compute_badge_groups
    _THREE_POINT_RULES = THREE_POINT_RULES
    _MID_POST_RULES = MID_POST_RULES
    _DRIBBLE_RULES = DRIBBLE_RULES
    _DEFENSE_RULES = DEFENSE_RULES
    
    _modules_loaded = True


def ensure_data_loaded(db_dir, project_root):
    """Lazy-load CSV data on first use."""
    global _rows, _name_index
    
    if _rows is not None:
        return
    
    ensure_modules_loaded(project_root)
    
    from generator_cli import load_rows
    _rows = load_rows(db_dir)
    
    _name_index = {}
    for r in _rows:
        name = normalize_name(repair_text(r.get("player_name", "")))
        if name:
            _name_index.setdefault(name, []).append(r)


def tendency_group(name):
    if name in _THREE_POINT_RULES:
        return "Shooting"
    if name in _MID_POST_RULES:
        return "Finishing"
    if name in _DRIBBLE_RULES:
        return "Playmaking"
    if name in _DEFENSE_RULES:
        return "Defense"
...
def generate_player_profile(player_name, season, project_root, roles_dir, badges_txt):
    """Generate profile for a single player."""
    target_name = normalize_name(player_name)
    target_season = season.strip().lower()

    season_matches = []
    for r in _name_index.get(target_name, []):
    ...
    all_player_rows = _name_index.get(player_name_clean, [])
    ...
    tendency_results = _compute_tendencies(source_row)
    source_row["season_label"] = season
    ml_result = _compute_attributes_ml(source_row, tendency_results, roles_dir, _rows)

    attrs = ml_result.get("attributes", {})
    roles = ml_result.get("roles", [])
    ovr = ml_result.get("ovr", 75)

    family_scores = _compute_attribute_family_averages(attrs)
    family_scores = {k: int(v) for k, v in family_scores.items()}

    badge_groups = _compute_badge_groups(source_row, attrs, tendency_results, family_scores, ovr, badges_txt)
...
def handle_generate(req):
    ...
    db_dir = req.get("db_dir", "")
    roles_dir = req.get("roles_dir", "")
    project_root = req.get("project_root", "")
    badges_txt = os.path.join(project_root, "Badges", "NBA 2K26 Badges.txt")
    
    if not players:
        print(json.dumps({"type": "error", "error": "No players provided"}), flush=True)
        return
    
    # Load data and modules if not already loaded
    print(json.dumps({"type": "progress", "status": "loading", "completed": 0, "total": len(players)}), flush=True)
    ensure_data_loaded(db_dir, project_root)
    ...
    for idx, player_name in enumerate(players):
        ...
        try:
            profile = generate_player_profile(player_name, season, project_root, roles_dir, badges_txt)
            print(json.dumps({"type": "player", "ok": True, "player": player_name, "profile": profile}), flush=True)
            success_count += 1
