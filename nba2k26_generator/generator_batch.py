"""Batch team profile generator.

Usage:
    python generator_batch.py <players_json> <season> <project_root> <db_dir> <roles_dir>

Outputs one JSON object per line to stdout (NDJSON format).
Each line is either:
    {"type": "player", "ok": true, "profile": {...}}
...
from generator_cli import (
    ...
    DRIBBLE_RULES,
    DEFENSE_RULES,
    normalize_player_name_for_match,
...
def stat_snapshot(r):
    ...
    fg_raw = first_non_empty(
        r.get("per_game_fg_percent"),
        r.get("per_game_fg_pct"),
        r.get("shooting_fg_percent"),
        r.get("shooting_fg_pct"),
        r.get("totals_fg_percent"),
        r.get("totals_fg_pct"),
        r.get("fg_percent"),
        r.get("fg_pct"),
    )
    ...
    fg3_raw = first_non_empty(
        r.get("per_game_x3p_percent"),
        r.get("per_game_fg3_pct"),
        r.get("shooting_fg_percent_from_x3p_range"),
        r.get("shooting_fg3_pct"),
        r.get("totals_x3p_percent"),
        r.get("totals_fg3_pct"),
        r.get("fg3_percent"),
        r.get("fg3_pct"),
    )
...
attribute_group_order = [
    ...
    (
        "Playmaking",
        ["Draw Foul", "Ball Handle", "Speed with Ball", "Hands", "Pass Accuracy", "Pass IQ", "Pass Vision"],
    ),
...
def load_career_csv():
    ...
    career_csv = os.path.join(project_root, "NBA Site data", "Carrer Stats", "alltime_leaders_per_game_regular_season.csv")
    if not os.path.isfile(career_csv):
        _career_cache = []
        return _career_cache
    import csv

    with open(career_csv, newline="", encoding="utf-8-sig") as f:
        _career_cache = list(csv.DictReader(f))
    return _career_cache


def get_career_override(row, nba_pid, player_name_lower):
    career_all = load_career_csv()
    if not career_all:
        return None
    matched = None
    if nba_pid:
        for cr in career_all:
            if str(cr.get("PLAYER_ID", "")).strip() == nba_pid:
                matched = cr
                break
    if matched is None:
        for cr in career_all:
            if cr.get("PLAYER_NAME", "").strip().lower() == player_name_lower:
                matched = cr
                break
    if matched is None:
        return None
    ...
    return {
        "pts": round(sf(matched.get("PTS")), 1),
        "reb": round(sf(matched.get("REB")), 1),
        "ast": round(sf(matched.get("AST")), 1),
        "stl": round(sf(matched.get("STL")), 1),
        "blk": round(sf(matched.get("BLK")), 1),
        "fgPct": round(sf(matched.get("FG_PCT")), 3),
        "fg3Pct": round(sf(matched.get("FG3_PCT")), 3),
        "ftPct": None if _sfn(matched.get("FT_PCT")) is None else round(_sfn(matched.get("FT_PCT")), 3),
        "gp": int(sf(matched.get("GP"))),
    }
...
for player_name in players:
    try:
        row = select_player_row_with_fallback(rows, player_name, season)
    except Exception as e:
        print(json.dumps({"type": "player", "ok": False, "player": player_name, "error": str(e)}), flush=True)
        continue

    player_name_clean = repair_text(row.get("player_name", "")).strip().lower()
    current_year = season_year(row.get("season_label", ""))
    player_rows = [r for r in rows if repair_text(r.get("player_name", "")).strip().lower() == player_name_clean]
    ...
    # Override career stats from dedicated CSV
    nba_pid = str(row.get("player_id", "")).strip()
    career_override = get_career_override(row, nba_pid, player_name_clean)
    if career_override is not None:
        career_snapshot = career_override
    ...
    try:
        ...
        unicorn_role = ml_result.get("unicorn_role", None)
        ovr = ml_result.get("ovr", compute_overall_rating(source_row.get("position", ""), attrs, compute_attribute_family_averages(attrs)))
        ...
        for family_name, names in attribute_group_order:
            attribute_groups[family_name] = [
                {"key": normalize_key(n), "name": n, "value": int(attrs.get(n, 0))} for n in names if n in attrs
            ]
        ...
        school_val = (
            first_non_empty(row.get("college"), row.get("school"), row.get("draft_college"), row.get("player_info_colleges"))
            or "NA"
        )
        ...
        print(json.dumps({"type": "player", "ok": True, "profile": payload}), flush=True)

    except Exception as e:
        print(json.dumps({"type": "player", "ok": False, "player": player_name, "error": str(e)}), flush=True)

print(json.dumps({"type": "done", "total": len(players)}), flush=True)
